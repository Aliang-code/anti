...
public class DateConversion {
    ...
    public static final String DEFAULT_DATETIME_WITHOUTSECOND = "yyyy-MM-dd HH:mm";
     ...
     * 获取num月后的日期
     */
    public static Date getDateAftXMonths(Date d, int num) {
        Calendar now = Calendar.getInstance();
        now.setTime(d);
     ...
     * 获取num年后的日期
     */
    public static Date getDateAftXYears(Date d, int num) {
        Calendar now = Calendar.getInstance();
        now.setTime(d);
    ...
    public static int getMonthsBetween(Date startDate, Date endDate) {
        if (null == startDate || null == endDate) {
            throw new ServiceException(ServiceException.VALUE_NEEDED, "startDate and endDate are needed");
        }
